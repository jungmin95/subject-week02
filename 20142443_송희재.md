...
1. Generate web.xml deployment descriptor를 클릭한다.
...
1. run을 돌려서 실행 -> 크롬으로 해당 경로 입력 후 실행창 뜨는지 확인
    - Run 실행
